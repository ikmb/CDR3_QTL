...
dir_analysis <- '../conditional_analysis/IBD/IBD_downsampled/all_sites_in_downsampled/'   
...
pairs_to_test <- grep(hit_pattern,paste0('../IBD/cdr3_hla_pairs_downsampled/', list.files('../IBD/cdr3_hla_pairs_downsampled/')), value = TRUE, invert = TRUE)
...
for (f in pairs_to_test){
    ...
    manova_df <- mlm_fun_conditional_old(cdr3_hla_condition_matrix, dir_results, sites) %>% 
        separate(pair, into = c('HLA', 'Site_hla', 'Length_cdr3', 'Position_cdr3'), sep = ':', remove = FALSE) %>% 
...
while (nrow(manova_df_site %>% filter(Pr..F.<bonf)) > 0){
    manova_results <- na.omit(fread(paste0(dir_analysis,'/manova_',condition_site,'.tsv'))) %>% 
        arrange(., Pr..F.,desc(variance_explained))
