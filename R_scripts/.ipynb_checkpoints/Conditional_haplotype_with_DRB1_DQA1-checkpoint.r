...
### Conditioning DQA1 based on DRB1_13
if (cond_round == 0){
    manova_cond_all <- na.omit(fread('/work_beegfs/sukmb667/projects/cdr3-qtl/healthy_and_ibd/conditional_analysis/without_groups_both/DRB1_conditional_round_2.tsv'))
    #[, Length_cdr3 := paste0('L', Length_cdr3)][, Site_hla := paste0(HLA, '_', Site_hla)]
    #setnames(manova_cond_all, old = grep('Pr', colnames(manova_cond_all), value = TRUE), new = 'Pvalue')
    significant_hits_with_length <- lapply(define_cond_hits(manova_cond_all), function(x) paste0('DRB1_', x))
} else {
    manova_cond_all <- fread(paste0(dir_results_cond, phenotype, '/', hla_gene, '_with_drb1_round_',cond_round, '.tsv'))
...
while (min_P < bonf){
    ...
    for (i in seq_along(significant_hits_with_length)){
        ...
        path_cond_manova <- paste0(dir_results_cond_phenotype, hla_gene, '_with_drb1_13_67_86_round_',length(significant_hits), '.tsv')
        ...
        for (j in seq_along(combinations_to_test)){
            ...
            dt2 <- dt[Site_hla_AA %like% sites[[1]] | Site_hla_AA %like% sites[[2]] | Site_hla_AA %like% sites[[3]] | Site_hla_AA %like% sites[[4]]]
